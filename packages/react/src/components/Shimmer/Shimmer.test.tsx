...
describe('Shimmer (react)', () => {
  ...
  it('renders children over the label fallback', () => {
    const { getByRole } = render(<Shimmer label="fallback">Click me</Shimmer>);
    expect(getByRole('button').textContent).toBe('Click me');
  });

  it('forwards a click handler', () => {
    const onClick = vi.fn();
    const { getByRole } = render(<Shimmer onClick={onClick}>Go</Shimmer>);
    fireEvent.click(getByRole('button'));
    expect(onClick).toHaveBeenCalledTimes(1);
  });

  it('reflects the disabled attribute on the native button', () => {
    const { getByRole } = render(<Shimmer disabled>Go</Shimmer>);
    expect((getByRole('button') as HTMLButtonElement).disabled).toBe(true);
  });

  it('maps props to CSS custom properties', () => {
    const { getByRole } = render(<Shimmer color="#fff" speed={5} />);
    const btn = getByRole('button');

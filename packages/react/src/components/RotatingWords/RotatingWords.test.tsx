...
describe('RotatingWords (react)', () => {
  it('exposes all words via aria-label', () => {
    const { container } = render(<RotatingWords words={['one', 'two', 'three']} />);
  ...
  it('maps word count + interval to CSS custom properties', () => {
    const { container } = render(<RotatingWords words={['one', 'two', 'three']} interval={5} />);

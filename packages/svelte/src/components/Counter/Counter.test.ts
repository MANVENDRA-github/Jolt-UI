...
describe('Counter (svelte)', () => {
  it('exposes the whole number via aria-label', () => {
    const { container } = render(Counter, { props: { value: 42 } });
  ...
  it('renders one aria-hidden rolling column per digit', () => {
    const { container } = render(Counter, { props: { value: 1990 } });
  ...
  it('zero-pads to the requested minimum column count', () => {
    const { container } = render(Counter, { props: { value: 7, digits: 3 } });
  ...
  it('maps duration + color to CSS custom properties', () => {
    const { container } = render(Counter, { props: { value: 1, duration: 2, color: '#fff' } });

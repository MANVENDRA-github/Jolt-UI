...
import Tactile from './Tactile.svelte';

describe('Tactile (svelte)', () => {
  ...
  it('forwards a click handler', async () => {
    const onclick = vi.fn();
    const { getByRole } = render(Tactile, { props: { onclick } });
    await fireEvent.click(getByRole('button'));
    expect(onclick).toHaveBeenCalledTimes(1);
  });

  it('reflects the disabled attribute on the native button', () => {
    const { getByRole } = render(Tactile, { props: { disabled: true } });

...
import Tactile from './Tactile.vue';

describe('Tactile (vue)', () => {
  it('renders a <button> with the label prop as fallback text', () => {
    const { getByRole } = render(Tactile, { props: { label: 'Go' } });
  ...
  it('renders slot content over the label fallback', () => {
    const { getByRole } = render(Tactile, {
  ...
  it('forwards a click handler via attribute fallthrough', async () => {
    const onClick = vi.fn();
    const { getByRole } = render(Tactile, { attrs: { onClick } });
    await fireEvent.click(getByRole('button'));
    expect(onClick).toHaveBeenCalledTimes(1);
  });

  it('reflects the disabled attribute on the native button', () => {
    const { getByRole } = render(Tactile, { attrs: { disabled: true } });
    expect((getByRole('button') as HTMLButtonElement).disabled).toBe(true);
  });

  it('maps props to CSS custom properties', () => {
    const { getByRole } = render(Tactile, { props: { color: '#fff', speed: 5 } });
    const btn = getByRole('button');

...
- **Current phase:** **Phase 7 — Loaders (the gallery's 3rd category).** v1 (Phases 0–4) + Phase 5 + **Phase 6 (a/b/c — 6 Three.js backgrounds incl. the Aurora shader) are merged to `main`**. **PR 7a — the Loaders category bootstrap + Spinner / Dot-Bounce / Bars** (a new `GRAPHIC` parity kind + a `role="status"` a11y pattern, D-034) is **merged to `main` (#31)**; **PR 7b — Pulse + Ripple** is **merged to `main` (#33)**. **PR 7c — Grid + Progress Bar** (the diagonal-wave grid + the indeterminate bar) is **built + all gates green locally; PR pending** — **3 categories, 23 components (10 text · 6 backgrounds · 7 loaders)**. *(The one remaining v1 deploy step is still YOURS: connect the repo in Cloudflare Pages — `pnpm build` → `apps/site/dist`, `NODE_VERSION=20`; see `DEPLOY.md`.)*
- **Repo:** `D:\Jolt-UI` · remote `github.com/MANVENDRA-github/Jolt-UI`. Branch → PR → merge (never push `main`).
- **Health:** `pnpm verify` green (**254 vitest + 45 `test:gen`** + registry:check incl. the `three`-isolation invariant, 25 items/framework) · `pnpm build` (36 pages, Pagefind-indexed) + `pnpm test:dist` · `pnpm test:cli` (**23 component items**, incl. the 6 backgrounds bundling `webgl-core` + `three`) · `pnpm test:e2e` (**15 tests**: on-page parity — text + the 7 loaders pixel-compared via the `GRAPHIC` kind — + a per-background isolation test over 6 backgrounds + install + SEO + docs + search + components-nav) green. The 7 loaders live-verified in a real browser (animate; reduced-motion static; `role="status"`; Loaders nav + index render; no console errors). (Phases 0–6 + 7a + 7b on `main`; 7c Grid+Progress Bar on `feat/phase-7c-grid-progress`, PR pending. CI green on every PR.)

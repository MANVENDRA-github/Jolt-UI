...
- **Current phase:** **Phase 7 — Loaders (the gallery's 3rd category).** v1 (Phases 0–4) + Phase 5 + **Phase 6 (a/b/c — 6 Three.js backgrounds incl. the Aurora shader) are merged to `main`**. **PR 7a — the Loaders category bootstrap + Spinner / Dot-Bounce / Bars** (a new `GRAPHIC` parity kind + a `role="status"` a11y pattern, D-034) is **merged to `main` (#31)**. **PR 7b — Pulse + Ripple** (2 more loaders on the proven CSS-graphic pattern) is **built + all gates green locally; PR pending** — **3 categories, 21 components (10 text · 6 backgrounds · 5 loaders)**. *(The one remaining v1 deploy step is still YOURS: connect the repo in Cloudflare Pages — `pnpm build` → `apps/site/dist`, `NODE_VERSION=20`; see `DEPLOY.md`.)*
- **Repo:** `D:\Jolt-UI` · remote `github.com/MANVENDRA-github/Jolt-UI`. Branch → PR → merge (never push `main`).
- **Health:** `pnpm verify` green (**242 vitest + 45 `test:gen`** + registry:check incl. the `three`-isolation invariant, 23 items/framework) · `pnpm build` (34 pages, Pagefind-indexed) + `pnpm test:dist` · `pnpm test:cli` (**21 component items**, incl. the 6 backgrounds bundling `webgl-core` + `three`) · `pnpm test:e2e` (**15 tests**: on-page parity — text + the 5 loaders pixel-compared via the `GRAPHIC` kind — + a per-background isolation test over 6 backgrounds + install + SEO + docs + search + components-nav) green. The 5 loaders live-verified in a real browser (animate; reduced-motion static; `role="status"`; Loaders nav + index render; no console errors). (Phases 0–6 + 7a on `main`; 7b Pulse+Ripple on `feat/phase-7b-pulse-ripple`, PR pending. CI green on every PR.)

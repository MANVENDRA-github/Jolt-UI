...
- **Two parity-harness robustness fixes** (the components render identically; these are test-only): the spec now (1) **settles** the larger set of `client:load` GSAP islands before screenshotting (the earliest-captured cell was otherwise caught mid-hydration), and (2) **hides Astro's dev toolbar**, which is fixed-position and bled into element screenshots as the page grew taller. See **D-018**.

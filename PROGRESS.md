...
- **New text components — Fade Up · Flip In · Neon (2026-07-05):** 3 CSS-only Text Animations added via the `gen-component` scaffolder + hand-customized keyframes, on `feat/text-fade-flip-neon` off `main` (one PR). **Fade Up** + **Flip In** are per-char (rise+fade / 3D rotateX flip, staggered); **Neon** is whole-text (a neon sign that flickers on to a steady glow, one-shot `forwards` so the settled state matches the reduced-motion static frame + the parity freeze → pixel-parity safe). Contracts saved at `scripts/contracts/{fade-up,flip-in,neon}.mjs`. **Text Animations now 13; the gallery is 6 categories… no — 5 categories, 35 components (13 text · 6 backgrounds · 7 loaders · 6 buttons · 3 cards).** Gotcha re-confirmed: the parity **harness** cells must render `text="Jolt UI"` (the spec's `TEXT` constant), not the contract's `harnessProps` label — fixed the 9 spliced cells + the contracts. Green: `pnpm verify` (**37 registry items/fw**) · `build` (48 pages) · `test:dist` · `test:cli` · `test:e2e` **17/17 twice**; live-verified (fade-up 710px / flip-in 481px motion-diff, neon `animation-name:jolt-neon` 1.6s + glow, all reduced-motion static diff 0, no component console errors).
- **UI/UX revamp — "Voltage 2: The Current" (2026-07-05, D-039):** the site-only 3D-landing + custom-scroll revamp — **5 stacked PRs #48→#52, pending merge** (branches stack `main → v2a → … → v2e`). Session-log entry + D-039 below. _(This text-components branch is off `main`, orthogonal to that stack.)_

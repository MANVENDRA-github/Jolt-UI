...
- **Determinism for tests:** animations are tested by *seeking* to a frame, never by wall-clock timing.
...
- **May NOT vary:** the look, the motion, the prop names/defaults/behavior. Anything a *user perceives* is single-sourced. If you find yourself changing motion in one skin only, lift it into `core`.
